...
import biasweave.release_artifacts as release_artifacts
from biasweave.release_artifacts import (
...
def rewrite_wheel(
    ...
    *,
    ...
    record_name = "biasweave-0.4.0.dist-info/RECORD"
...
def spdx_document(*, package_name: str = NAME) -> dict[str, object]:
    package_id = "SPDXRef-Package-python-biasweave"
    file_id = "SPDXRef-File-biasweave-init-py"
    return {
        ...
        "files": [
            {
                "fileName": "biasweave/__init__.py",
                "SPDXID": file_id,
...
@pytest.mark.parametrize("package_name", ["biasweave", "BiasWeave"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        f"pkg:pypi/biasweave-other@{VERSION}",
        None,
        [],
    ],
)
...
def raw_syft_document() -> dict[str, object]:
    ...
    return {
        ...
        "files": [
            {"fileName": "/biasweave/__init__.py", "SPDXID": "SPDXRef-File-code"},
            {
                "fileName": "/biasweave-0.4.0.dist-info/METADATA",
                "SPDXID": "SPDXRef-File-metadata",
            },
            {
                "fileName": "/biasweave-0.4.0.dist-info/RECORD",
...
def installed_tree(directory: Path) -> tuple[Path, Path]:
    package = directory / "biasweave"
    package.mkdir(parents=True)
    init_payload = b""
    (package / "__init__.py").write_bytes(init_payload)
    dist_info = directory / "biasweave-0.4.0.dist-info"
    dist_info.mkdir()
    ...
    record_payload = (
        row("biasweave/__init__.py", init_payload)
        + row("biasweave-0.4.0.dist-info/METADATA", metadata_payload)
        + "biasweave-0.4.0.dist-info/RECORD,,\n"
    ).encode()
    ...
    with zipfile.ZipFile(wheel, "w") as archive:
        archive.writestr("biasweave/__init__.py", init_payload)
        archive.writestr("biasweave-0.4.0.dist-info/METADATA", metadata_payload)
        archive.writestr("biasweave-0.4.0.dist-info/RECORD", record_payload)
    ...
    [
        ...
        (
            lambda record: record.replace(
                b"biasweave-0.4.0.dist-info/RECORD,,",
                b"biasweave-0.4.0.dist-info/RECORD,sha256=bad,1",
            ),
            "self-hash",
        ),
        (
            lambda record: record.replace(
                b"biasweave/__init__.py,sha256=",
                b"biasweave/__init__.py,md5=",
            ),
            "lacks SHA-256",
        ),
        (
        ...
        (
            lambda record: (
                record + b"biasweave/missing.py,sha256="
        ...
        (
            lambda record: record.replace(b"biasweave/__init__.py", b"biasweave/../__init__.py", 1),
            "non-canonical path",
        ),
    ],
)
def test_wheel_record_rejects_malformed_claims(tmp_path: Path, transform, message: str) -> None:
    ...
    with pytest.raises(ReleaseArtifactError, match=message):
        release_artifacts._wheel_distribution(
            wheel, NAME, VERSION, "biasweave", "biasweave-0.4.0.dist-info"
        )
...
def test_wheel_inspection_rejects_bad_archive_shapes(tmp_path: Path) -> None:
    ...
    with pytest.raises(ReleaseArtifactError, match="exact regular"):
        release_artifacts._wheel_distribution(
            wrong_name, NAME, VERSION, "biasweave", "biasweave-0.4.0.dist-info"
        )
    ...
    with pytest.warns(UserWarning, match="Duplicate name"), zipfile.ZipFile(wheel, "a") as archive:
        archive.writestr("biasweave/__init__.py", b"duplicate")
...
def test_wheel_inspection_rejects_symbolic_link_entry(tmp_path: Path) -> None:
    _site_packages, wheel = installed_tree(tmp_path / "site-packages")
    link = zipfile.ZipInfo("biasweave/link.py")
...
def test_wheel_record_must_contain_the_project_package(tmp_path: Path) -> None:
    ...
    metadata_name = "biasweave-0.4.0.dist-info/METADATA"
    record_name = "biasweave-0.4.0.dist-info/RECORD"
    metadata = b"Name: biasweave\nVersion: 0.4.0\n"
...
def test_installed_distribution_rejects_missing_duplicate_and_bad_record(tmp_path: Path) -> None:
    site_packages, wheel = installed_tree(tmp_path / "site-packages")
    metadata = site_packages / "biasweave-0.4.0.dist-info" / "METADATA"
    ...
    duplicate.mkdir()
    (duplicate / "METADATA").write_text("Name: biasweave\nVersion: 0.4.0\n", encoding="utf-8")
    with pytest.raises(ReleaseArtifactError, match="exactly one"):
        release_artifacts._installed_distribution(site_packages, wheel, NAME, VERSION)

    site_packages, wheel = installed_tree(tmp_path / "third-site-packages")
    record = site_packages / "biasweave-0.4.0.dist-info" / "RECORD"
...
def test_installed_distribution_binds_allowlisted_external_script(tmp_path: Path) -> None:
    ...
    script = installation_root / "bin" / "biasweave"
    script.parent.mkdir()
    ...
    with record.open("a", encoding="utf-8", newline="") as stream:
        stream.write(f"../bin/biasweave,sha256={encoded},{len(script_payload)}\n")
    document = raw_syft_document()
    ...
    document["files"].append(  # type: ignore[union-attr]
        {"fileName": "/bin/biasweave", "SPDXID": "SPDXRef-File-cli"}
    )
    sbom = tmp_path / "sbom.json"
    sbom.write_text(json.dumps(document), encoding="utf-8")
    ...
    assert (
        bind_installed_wheel(
            ...
            expected_external_paths=("bin/biasweave",),
        )
        == 4
    )
    validate_spdx_path(
    ...
    assert set(files) == {
        "bin/biasweave",
        "site-packages/biasweave/__init__.py",
        "site-packages/biasweave-0.4.0.dist-info/METADATA",
        "site-packages/biasweave-0.4.0.dist-info/RECORD",
    }
...
def test_installed_distribution_rejects_external_escape_and_bad_claim(tmp_path: Path) -> None:
    ...
    launcher = installation_root / "bin" / "biasweave"
    launcher.parent.mkdir()
    launcher.write_bytes(b"launcher")
    record.write_text(
        record.read_text(encoding="utf-8").replace(
            "../../outside-launcher,,", "../bin/biasweave,sha256=bad,8"
        ),
        encoding="utf-8",
    )
    with pytest.raises(ReleaseArtifactError, match="invalid SHA-256"):
...
@pytest.mark.parametrize(
    ("line", "message"),
    [
        ("biasweave\\__init__.py,,\n", "not portable"),
        ("biasweave/__init__.py,,\n", "aliases an installed path"),
        ("biasweave/missing.py,,\n", "missing"),
    ],
)
...
def test_installed_record_rejects_a_self_hash(tmp_path: Path) -> None:
    ...
    record.write_text(
        record.read_text(encoding="utf-8").replace(
            "biasweave-0.4.0.dist-info/RECORD,,",
            "biasweave-0.4.0.dist-info/RECORD,sha256=bad,1",
        ),
        encoding="utf-8",
    )
...
def test_installed_record_read_failure_is_actionable(tmp_path: Path) -> None:
    site_packages, wheel = installed_tree(tmp_path / "site-packages")
    (site_packages / "biasweave-0.4.0.dist-info" / "RECORD").unlink()
...
def test_bind_installed_wheel_rejects_record_content_mismatch(tmp_path: Path) -> None:
    site_packages, wheel = installed_tree(tmp_path / "site-packages")
    (site_packages / "biasweave" / "__init__.py").write_bytes(b"tampered")
    sbom = tmp_path / "sbom.json"
    sbom.write_text(json.dumps(raw_syft_document()), encoding="utf-8")
...
def test_bind_installed_wheel_rejects_unrecorded_wheel_entry(tmp_path: Path) -> None:
    site_packages, wheel = installed_tree(tmp_path / "site-packages")
    with zipfile.ZipFile(wheel, "a") as archive:
        archive.writestr("biasweave/unrecorded_probe.py", b"probe = True\n")
    sbom = tmp_path / "sbom.json"
    sbom.write_text(json.dumps(raw_syft_document()), encoding="utf-8")
...
@pytest.mark.parametrize(
    "rogue_path",
    ["rogue.py", "biasweave-0.4.0.data/purelib/rogue.py"],
)
...
def test_bind_installed_wheel_rejects_unrecorded_installed_file(tmp_path: Path) -> None:
    site_packages, wheel = installed_tree(tmp_path / "site-packages")
    (site_packages / "biasweave" / "post_install_probe.py").write_text(
        "probe = True\n", encoding="utf-8"
    )
    sbom = tmp_path / "sbom.json"
    sbom.write_text(json.dumps(raw_syft_document()), encoding="utf-8")
...
@pytest.mark.parametrize(
    "alias_template",
    [
        "biasweave/../biasweave/__init__.py",
        "../{site_packages}/biasweave/__init__.py",
    ],
)
...
def test_bind_installed_wheel_measures_installer_generated_pyc(tmp_path: Path) -> None:
    site_packages, wheel = installed_tree(tmp_path / "site-packages")
    cache = site_packages / "biasweave" / "__pycache__"
    cache.mkdir()
    generated = cache / "__init__.cpython-311.pyc"
    generated.write_bytes(b"installer generated")
    record = site_packages / "biasweave-0.4.0.dist-info" / "RECORD"
    with record.open("a", encoding="utf-8", newline="") as stream:
        stream.write("biasweave/__pycache__/__init__.cpython-311.pyc,,\n")
    document = raw_syft_document()
    document["files"].append(  # type: ignore[union-attr]
        {
...
def test_bind_installed_wheel_verifies_hashed_installer_metadata(tmp_path: Path) -> None:
    site_packages, wheel = installed_tree(tmp_path / "site-packages")
    dist_info = site_packages / "biasweave-0.4.0.dist-info"
    generated_payloads = {
        "INSTALLER": b"pip\n",
    ...
    with record.open("a", encoding="utf-8", newline="") as stream:
        for filename, payload in generated_payloads.items():
            ...
            stream.write(f"biasweave-0.4.0.dist-info/{filename},sha256={encoded},{len(payload)}\n")
    document = raw_syft_document()
    for filename in generated_payloads:
        document["files"].append(  # type: ignore[union-attr]
            {
                "fileName": f"/biasweave-0.4.0.dist-info/{filename}",
...
def test_bind_installed_wheel_rejects_bad_installer_metadata_claim(
    ...
    record = dist_info / "RECORD"
    with record.open("a", encoding="utf-8", newline="") as stream:
        stream.write(f"biasweave-0.4.0.dist-info/INSTALLER,{record_hash},{record_size}\n")
    document = raw_syft_document()
    document["files"].append(  # type: ignore[union-attr]
        {
            "fileName": "/biasweave-0.4.0.dist-info/INSTALLER",
...
def test_bind_installed_wheel_rejects_bad_syft_file_and_relationship_shapes(tmp_path: Path) -> None:
    ...
    duplicate["files"].append(  # type: ignore[union-attr]
        {"fileName": "/biasweave/__init__.py", "SPDXID": "SPDXRef-File-copy"}
    )
